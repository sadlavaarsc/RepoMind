...
def generate_summary_report(all_results: List[Dict[str, Any]], output_path: Path):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        for repo in ["travel_agent", "cuezero"]:
            ...
            # Metrics table with quality
            f.write("| 系统 | 平均召回率 | 平均命中率 | 平均精确率 | 平均答案质量 | 平均延迟(ms) | 平均总Token |\n")
            f.write("|------|-----------|-----------|-----------|------------|------------|-----------|\n")

            for result in repo_results:
                agg = result["aggregated"]
                perf = result["performance"]
                f.write(f"| {result['system']} | {agg.get('avg_recall', 0):.3f} | {agg.get('avg_hit_rate', 0):.3f} | {agg.get('avg_precision', 0):.3f} | {result.get('avg_quality_score', 0):.3f} | {perf.get('avg_latency_ms', 0):.1f} | {perf.get('avg_total_tokens', 0):.0f} |\n")
        ...
        # Overall comparison
        f.write("## 总体对比\n\n")
        f.write("| 项目 | 系统 | 平均召回率 | 平均命中率 | 平均精确率 | 平均答案质量 | 平均延迟(ms) | 平均总Token |\n")
        f.write("|------|------|-----------|-----------|-----------|------------|------------|-----------|\n")

        for result in all_results:
            agg = result["aggregated"]
            perf = result["performance"]
            f.write(f"| {result['repo']} | {result['system']} | {agg.get('avg_recall', 0):.3f} | {agg.get('avg_hit_rate', 0):.3f} | {agg.get('avg_precision', 0):.3f} | {result.get('avg_quality_score', 0):.3f} | {perf.get('avg_latency_ms', 0):.1f} | {perf.get('avg_total_tokens', 0):.0f} |\n")
